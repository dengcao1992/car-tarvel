...
private[hbase] case class HbaseRelation(@transient val hbaseProps:Map[String , String])
  ...
  //2你查询表的rowkey范围是什么
  private val rowRange: String = hbaseProps.getOrElse("->", "->")
  private val range: Array[String] = rowRange.split("->")
  val startRowkey = range(0).trim
  val endRowkey = range(1).trim

  //3:获取 hbase要查询的列
  val hbaseTableSchema = hbaseProps.getOrElse("hbase_table_name" , sys.error(" 获取不到查询的hbase列"))
  //4:获取sprk的
